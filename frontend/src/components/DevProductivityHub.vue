...
async function saveCurrentNote() {
  ...
  try {
    ...
    const { data } = await axios.put(`/api/notes/${selectedNote.value.id}`, {
      title:       selectedNote.value.title,
      content:     selectedNote.value.content,
      deadline_at: selectedNote.value.deadline_at || null,
    })
    const idx = notes.value.findIndex(n => n.id === data.data.id)
    if (idx !== -1) notes.value[idx].updated_at = data.data.updated_at
    lastSaved.value = true
    // Hilangkan tanda "Tersimpan" setelah 2 detik
...
async function completePomodoro() {
  ...
  try {
    // PUT /api/notes/:id/pomodoro — increment total_pomodoros di DB
    const { data } = await axios.put(`/api/notes/${targetId}/pomodoro`)
